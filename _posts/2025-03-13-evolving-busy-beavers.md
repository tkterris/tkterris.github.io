...
The intuitive reason why the proof provides better bounds than Theorem 20[^1] comes from the fact that that Chaitin's constant is "overpowered" for determining whether a program halts. The first *n+1* bits of Chaitin's constant, *Ω<sub>n+1</sub>*, can determine the number of halting programs of length *n+1*, *H<sub>n+1</sub>*, and therefore *BB<sub>L</sub>(n+1)*. However, *Ω<sub>n+1</sub>* also encodes information about much larger programs. *Ω<sub>n+1</sub>* tells you *H<sub>n+2</sub>* within a factor of 2, *H<sub>n+3</sub>* within a factor of 4, and so on. That's a lot to ask from just *BB<sub>L</sub>(n)*, requiring *O(log n)* advice bits to make up the difference to generate *Ω<sub>n+1</sub>*. However, by focusing solely on *H<sub>n+1</sub>* rather than later values of *H*, we reduce the "level of effort". Usually, anyway -- if *H<sub>n+1</sub>* is very large, then that *does* tell you something about *H<sub>n+2</sub>* and so on (because the language is prefix-free, a large number of halting programs of length *n+1* "rules out" many longer programs), which means you would need those additional advice bits. However, it turns out that the probability of large *H<sub>n+1</sub>* asymptotically approaches 0.
...
TODO: enc(X) needs to be part of L
TODO: generic Min_enc rather than 7

First, some notation: 
- *H<sub>x</sub>* is the number of halting L-programs of length *x* bits (program plus input). 
...
TODO: bounds because L is not necessarily self-delimiting
...
#### <span id="lemma2">Lemma 2: There is a procedure that can be used to determine *BB(n+1)* from *BB(n)* and a number of advice bits, *p*, such that *p <= log(H<sub>n+1</sub>) + O(log log n) + \|enc(n)\| + \|enc(p)\| - n*</span>

TODO: introduction and reasoning

Consider the following pseudocode:

```
doesThisManyHalt(int candidate, int p, int padding) {
    if (candidate == 0) {
        exit;
    }
    int n = |doesThisManyHalt| + |enc(candidate)| + |enc(p)| + padding;
    int halted = 0;
...
__Inputs and total program size:__ *doesThisManyHalt* takes a prefix-free encoding of a candidate value for *H<sub>n+1</sub>* (with *p* least-significant bits truncated from the candidate), a prefix-free encoding of the number of truncated bits *p*, and a prefix-free encoding of the number *padding*. The full length of the L-program is therefore: 
- *length = \|doesThisManyHalt\| + \|enc(candidate)\| + \|enc(p)\| + \|enc(padding)\|* 

__Program logic:__ First, the special case where *candidate = 0* is checked, and immediately halts if so. Then *n* is inferred from the length of the program definition (a hard-coded constant), the lengths of the prefix-free encodings of *candidate* and *p*, and value of *padding*:
- *n = \|doesThisManyHalt\| + \|enc(candidate)\| + \|enc(p)\| + padding*

With [Levenshtein coding](https://en.wikipedia.org/wiki/Levenshtein_coding), *x < \|enc(x)\|* for all *x < 7*. We will be keeping the length of this program less than or equal to *n*. So, given the length of the program and the computation of *n*, both given above, *padding* cannot be less than 7.

It then iterates through each of the *2<sup>n+1</sup>* program strings of length *n+1*, emulating them in parallel. Whenever one of the programs halts, it is added to a tally *halted*. If that tally reaches the input candidate multiplied by *2<sup>p</sup>*, then *doesThisManyHalt* halts. 

__Estimating H<sub>n+1</sub>:__ Suppose we know *BB(n)*. Because *doesThisManyHalt* plus its inputs are length less than *n*, we can evaluate whether it halts. This can be used in a test to estimate *H<sub>n+1</sub>* given *BB(n)*: 
1. Start with *candidate = 0* and *p = 0*.
1. Run *doesThisManyHalt* with inputs *candidate*, *p*, and *padding = n - \|doesThisManyHalt\| - \|enc(candidate)\| - \|enc(p)\|*. Use *BB(n)* to determine if it halts.
1. If *doesThisManyHalt* halts, *H<sub>n+1</sub> >= candidate \* 2<sup>p</sup>*. 
  1. Increment *candidate* by one. 
  1. Compute the new padding: *padding = n - \|doesThisManyHalt\| - \|enc(candidate)\| - \|enc(p)\|*
  1. If *padding < 7*, the encoding of *padding* will be longer than *padding* itself, putting the program length over *n*. Reset *candidate* to zero and increment *p* instead. 
  1. Return to the previous step to rerun *doesThisManyHalt*.
1. If *doesThisManyHalt* does not halt, *H<sub>n+1</sub> < candidate \* 2<sup>p</sup>*.

Because the input *p* is adjustable rather than hard-coded, *candidate\*2<sup>p</sup>* can get to *2<sup>n+1</sup>*, the worst-case upper bound of *H<sub>n+1</sub>*, with only *O(log n)* bits of *p*. That is, for all *n >= \|doesThisManyHalt\| + \|enc(1)\| + \|enc(n+1)\| + 7*, we will eventually find inputs such that *doesThisManyHalt* does not halt. 
...
If these *p* bits are provided as advice bits, we will have the exact value of *H<sub>n+1</sub>*. We can then run all programs of length *n+1* in parallel until that many halt, and we can select the longest-running such program as *BB(n+1)*.

__Upper bounds on *padding*:__ Consider the inputs *(candidate, p, padding)* that resulted in the first non-halting run of *doesThisManyHalt*. Recall that: 
- *n = \|doesThisManyHalt\| + \|enc(candidate)\| + \|enc(p)\| + padding*

Suppose this *p* is nonzero (if it were zero, there would be no advice bits needed, and *BB(n+1)* would be found). This implies:
- *n < \|doesThisManyHalt\| + \|enc(2 \* candidate)\| + \|enc(p - 1)\| + 7*

That is, adding a single bit to the candidate must have resulted in a program (with minimum possible *padding*) that couldn't fit into *n* bits. Otherwise, that additional candidate bit would have been found. 

So, we have:
- *\|doesThisManyHalt\| + \|enc(candidate)\| + \|enc(p)\| + padding < \|doesThisManyHalt\| + \|enc(2 \* candidate)\| + \|enc(p - 1)\| + 7* from the above statements
- *padding < \|enc(2 \* candidate)\| - \|enc(candidate)\| + \|enc(p - 1)\| - \|enc(p)\| + O(1)* by canceling constants and moving terms
- *padding < \|enc(2 \* candidate)\| - \|enc(candidate)\| + O(1)* because encoding lengths are monotonically increasing
- *padding < log(candidate) + O(log(log((2\*candidate)))) - log(candidate) - O(log(log(candidate)))* from the fact that *\|enc(x)\| = log(x) + O(log(log(x)))*
- *padding < O(log(log(candidate)))*

For Levenshtein coding in particular, the actual upper bound is *O(log\*(log(candidate)))*, where *log\*(x)* is the [iterated logarithm](https://en.m.wikipedia.org/wiki/Iterated_logarithm). However, we can relax this condition and still get the same end result.

__Upper bounds on *p*:__ First, we get a bound on the (non-prefix-free) number of candidate bits, *log(candidate)*, for any run of *doesThisManyHalt* in terms of *n* and *p*. Starting with the computation of *n* from "Program logic" above:
- *n = \|doesThisManyHalt\| + \|enc(candidate)\| + \|enc(p)\| + padding*
- *n < O(1) + log(candidate) + \|enc(log(candidate))\| + \|enc(p)\| + O(log(log(candidate)))*
- *log(candidate) = n - O(log(log(candidate))) - \|enc(log(candidate))\| - \|enc(p)\|*
- *log(candidate) >= n - O(log(log(n))) - \|enc(n)\| - \|enc(p)\|*
...
- *p <= log(H<sub>n+1</sub>) + 1 - (n - O(log(log(n))) - \|enc(n)\| - \|enc(p)\| - padding)*
- *p <= log(H<sub>n+1</sub>) + O(log(log(n))) + \|enc(n)\| + \|enc(p)\| + padding - n* ∎
...
Another potential simplification would be if the language *L* were self-delimited -- we could have just passed in the prefix-free *n* and *p*, and used the rest of the bitstring for a non-prefix-free encoding of *candidate*. *doesThisManyHalt* would just read bits one by one, having inferred the length of the *candidate* bits from the other inputs. Self-delimiting is a basically unknown feature (outside of the literature[^5] anyway), so that would have been a significant constraint on *L* to which this proof applies.
...
- *p <= log(H<sub>n+1</sub>) + O(log(log(n))) + \|enc(n)\| + \|enc(p)\| - n*
- *p < log(2^(n+1) / (n+1)) + O(log(log(n))) + \|enc(n)\| + \|enc(p)\| - n* 
- *p < (n + 1) - log(n+1) + log(n) + O(log(log(n))) + \|enc(p)\| - n*
- *p < O(log(log(n))) + \|enc(p)\|*
- *p < O(log(log(n)))*  ∎
...
[^5]: Chaitin, G.J. (1995). [The Limits of Mathematics---Tutorial Version](https://arxiv.org/abs/chao-dyn/9509010). arXiv: Chaotic Dynamics.
