...
[^4]: To be clear, it is certainly possible for there to be infinitely many *n* which break this condition, in which case there is no constant *D* which can be used as a correction. Consider the prefix-free language which is defined: 
    ...
    haltingBits  = log(number of halting programs) + 1
                ...
                >= n - log(n)
    ```
    which means that for *n=2<sup>b</sup>*, the bits needed to count the number of halting programs is greater than *n + 2 - len(enc(n))* by *O(log(log(n)))* for infinitely many *n*. However, such breaking *n* occur with probability *n<sup>-1</sup>log(n)*, which asymptotically approaches zero. It seems unlikely that a "sensible" (whatever that means) language would have such irregular spikes in the number of halting programs, but the fact that it can theoretically happen infinitely many times means that this proof only applies for "almost all" *n*.
